...
install_python_tools() {
    ...
    for tool in "${PIP_TOOLS[@]}"; do
        if pip3 show "$tool" &>/dev/null || command -v "$tool" &>/dev/null; then
            ...
            pip3 install "$tool" --break-system-packages -q 2>/dev/null && info "$tool installed" || warn "Failed to install $tool"
        fi
    done

    if [ ! -d "/opt/LinkFinder" ]; then
        git clone -q https://github.com/GerbenJavado/LinkFinder.git /opt/LinkFinder 2>/dev/null || true
        cd /opt/LinkFinder && pip3 install -r requirements.txt --break-system-packages -q 2>/dev/null && cd - >/dev/null
    ...
    if [ ! -d "/opt/paramspider" ]; then
        git clone -q https://github.com/devanshbatham/paramspider /opt/paramspider 2>/dev/null || true
        cd /opt/paramspider && pip3 install -r requirements.txt --break-system-packages -q 2>/dev/null && cd - >/dev/null
        chmod +x /opt/paramspider/paramspider.py 2>/dev/null || true
        ln -sf /opt/paramspider/paramspider.py /usr/local/bin/paramspider 2>/dev/null || true
        info "paramspider installed"
    else
        echo -e "[${BCYAN}$(timestamp)${RESET}] [${DIM}dbg${RESET}] paramspider already installed"
    fi
    ...
    # Apt tools
    for tool in amass massdns dnsrecon testssl.sh whatweb dirsearch theharvester wafw00f; do
        if ! command -v "$tool" &>/dev/null; then
